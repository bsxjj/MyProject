...
<jsp:include page="./head.jsp"/>
 ...
 <div class="mainContent">
    <aside class="aside-left">
      <div class="avatar">
        <a href="${ pageContext.request.contextPath}/user?method=myinfo" style="background: url(${user.headPicture}) no-repeat center;background-size: 220px 220px;"><span>${ user.username}</span></a>
      </div>
      <section class="topspaceinfo">
        
        <p class="small">${ user.signature}</p>
      </section>
      <div class="userinfo"> 
        <p class="q-fans" id="fansnum" style="color:#f16e50;">粉丝：167</p> 
        <p class="btns"><a href="${pageContext.request.contextPath}/view/notifications.jsp" target="_blank" >私信</a><a href="${pageContext.request.contextPath}/view/album.jsp" target="_blank">相册</a><a href="/" target="_blank">留言板</a></p>   
      </div>
      <section class="newpic">
         <h2>最新照片</h2>
         <ul>
         
          <c:forEach var="img" items="${user.imgs}">
      ...
      <section class="taglist">
         ...
         <ul id="interstlist">
           ...
           <c:forEach var="intern" items="${user.intersts}">
    ...
    <div class="write-imgtxt">
    	<span class="write-img-head">快来分享你的图片!</span>
    	<textarea rows="5" cols="20" style="border-color:#f16e50;overflow:hidden; resize:none;" name="imgtxt-txt"></textarea>
    	<span>
    	<img id="toggle-file" src="${ pageContext.request.contextPath}/images/icon_img.png" style="width=30px;height:30px;">
    	<button class="btn btn-danger" id="send-imgtxt">发表图文</button>
    	</span>
    ...
    <div class="thumbnail">
      ...
      <div class="footer_star">
		<ul class="stats">
		<li><a href="javascript:;" class="collection_num"><span class="glyphicon glyphicon-heart"></span>10&nbsp;&nbsp;&nbsp;&nbsp;</a></li>
		<li><a href="javascript:;" class="comment_num"><span class="glyphicon glyphicon-comment"></span>128&nbsp;&nbsp;&nbsp;&nbsp;</a></li>
		<li><a href="javascript:;" class="laud_num"><span class="glyphicon glyphicon-thumbs-up"></span>152&nbsp;&nbsp;&nbsp;&nbsp;</a></li>
		</ul>
	 </div>
      </div>
       <div class="write-comment">
       	<span><input type="hidden" name="imgtxtid" value="123" class="imgtxtid"></span>
    	<span><textarea class="comment_content" rows="2" style="width:100%;border-color:#f16e50;overflow:hidden; resize:none;" placeholder="发表你的评论吧!"></textarea></span>
  ...
  <%-- <div class="col-sm-12 col-md-12" style="padding-top: 10px;margin:0 auto">
    <div class="thumbnail">
      ...
      <div class="write-comment">
    	<span><textarea rows="2" style="width:100%;border-color:#f16e50;overflow:hidden; resize:none;" placeholder="发表你的评论吧!"></textarea></span>
    	<span><button class="btn btn-danger right-button pull-right">发表</button></span>
    </div>
    </div>
  </div>
   --%>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/comments.js">
</script>
...
<script type="text/javascript">
	...
	function setImgTxtHtml(pageNum){
		$.ajax({
			type:"GET",
			url:"http://localhost:8080/timer/imgtxt?method=getalltxt",
	...
	//发表图文消息
	$("#send-imgtxt").click(function(){
		var imgdata = $("input[name='imagebase64']").val();
		var imgtxt = $("textarea[name='imgtxt-txt']").val();
		imgtxt = imgtxt.trim();
		//alert(imgtxt);
		...
		$.ajax({
			type:"POST",
			url:"http://localhost:8080/timer/imgtxt?method=saveimgtxt",
			data:"imgdata=" + imgdata +"&imgtxt="+imgtxt,
			success:function(msg){
				alert(msg);
				$("input[name='imagebase64']").attr("value","");
				$("textarea[name='imgtxt-txt']").val("");
			}
   ...
   /* 	$("#prevpage").on("click",);
	...
	var isWrittenShow = false;
   	$(function(){
   		...
   		$.ajax({
			type:"GET",
			url:"http://localhost:8080/timer/interest?method=allinter",
			success:function(msg){
				if (msg != null && msg != ""){
					var interArry = jQuery.parseJSON(msg);
					var allhtml = "";
					for (var i = 0; i < interArry.length;i++){
						allhtml = allhtml + " <li><a href=\"/\"><span class=\"label label-warning\">"+ interArry[i].interstName +"</span></a></li>"
					}
					$("#interstlist").html(allhtml);
   		...
   		$.ajax({
			type:"GET",
			url:"http://localhost:8080/timer/follow?method=getfans",
			success:function(msg){
				if (msg != null && msg != ""){
					//alert(msg);
   		...
   		$.ajax({
			type:"GET",
			url:"http://localhost:8080/timer/imgtxt?method=getcount",
			success:function(msg){
				if (msg != null && msg != ""){
					//alert(msg);
					var obj = jQuery.parseJSON(msg);
