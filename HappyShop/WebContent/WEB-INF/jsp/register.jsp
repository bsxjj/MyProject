...
<script type="text/javascript">
	...
	function checkpwd() {
		...
		//使用正则表达式验证
		var RegPwd = /^\d{6,16}$/;
		var pwd_prompt = document.getElementById("password_prompt");
		if (RegPwd.test(pwd) == false) {
			pwd_prompt.innerHTML = "密码必须是6-16";
			pwd_prompt.className = "prompt";
			return false;
		} else {
			pwd_prompt.innerHTML = "";//提示内容清空
			return true;
		}
	}
	//验证邮箱
	function checkEmail() {
		...
		if (regEmail.test(email) == false) {
			email_prompt.innerHTML = "邮箱必须包含@和.";
			email_prompt.className = "prompt";
			return false;
		} else {
		...
		if(val!=""){
			...
			$.post(url,args,function(data){
			  ...
			  if(data=='ok'){
				 ...
				 $("#username_prompt").html("<font color='green'>该用户可以被注册</font>");
			  }
			  else{
				  
				$("#username_prompt").html("<font color='red'>该用户已被注册，请重新注册一个</font>");
			  }
			});
		}
	}
...
<%@include file="menu.jsp" %>
							...
							<tbody>
								...
								<tr>
									...
									<td><span class="fieldSet"> <input type="text"
											...
											src="${pageContext.request.contextPath}/checkCode" onclick="this.src=this.src+'?'" title="点击更换验证码">
									    </span>
									</td>
								</tr>
								<tr>
	...
	<div class="container footer">
		...
		<div class="span24">
			<div class="copyright">Copyright © 2017网上商城 版权所有</div>
		</div>
	</div>
	<div id="_my97DP"
